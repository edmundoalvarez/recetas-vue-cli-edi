<template>
    <div class="container">
      <h1 class="brown--text">Editá la receta</h1>
        <v-form ref="form">
    ...
    export default {
      ...
      methods: {
        guardar: function (form_data) {
          ...
          if (this.errores.length == 0) {
            ...
            this.recetasCargadas.indexOf(form_data.id);
            localStorage.setItem("form", JSON.stringify(this.recetasCargadas));
            this.$router.push("/recetasguardadas");
            console.log(this.recetasCargadas)
          }
        },

        recetaSeleccionada: function (){
            if(this.recetasCargadas.length > 0) {

                for (const d of this.recetasCargadas) {
      ...
      mounted: function(){
        ...
        if (!localStorage.form) {
        this.recetasCargadas = [];
        
        } else {
        this.recetasCargadas = JSON.parse(localStorage.getItem("form"));
